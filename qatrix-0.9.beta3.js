...
var Qatrix = {
	...
	$data = {
		get: function (elem, name)
		{
			...
			return value === "true" ? true : value === "false" ? false : value === "null" ? '' : value === null ? '' : value === '' ? '' : Qatrix.rdigit.test(value) ? parseInt(value) : $json.isJSON(value) ? $json.decode(value) : value;
		},
		set: function (elem, name, value)
		{
	...
	$className = {
		add: function (elem, name)
		{
			...
			else
			{
				...
				{
					var haystack = elem.className.split(/\s+/),
						match;
					$each(haystack, function (i, item)
					{
						if (item === name)
						{
							match = true;
		...
		{
			return new RegExp('\\b(' + name.split(' ').join('|') + ')\\b').test(elem.className);
		},
		remove: function (elem, name)
		{
			...
			setTimeout(function ()
			{
				...
				if (properties['left'] || properties['top'])
				{
					var transform = $string.camelCase(prefix_name + 'transform'),
						offset = $offset(elem);
					translate3d = (properties['left'] && properties['top']) ? 'translate3d(' + (property_to_value['left'] - offset.left) + unit['left'] + ',' + (property_to_value['top'] - offset.top) + unit['top'] + ',0)' : (properties['left']) ? 'translate3d(' + (property_to_value['left'] - offset.left) + unit['left'] + ',0,0)' : 'translate3d(0,' + (property_to_value['top'] - offset.top) + unit['top'] + ',0)';
