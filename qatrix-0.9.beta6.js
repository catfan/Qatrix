...
var Qatrix = {
	...
	}()) ? (function ()
	{
		...
		return function (elem, properties, duration, callback)
		{
			...
			AUID = 'Qani_' + Math.random().toString().replace('0.', '');
			rule = '.' + AUID + '{' + transition_name + ':all ' + duration + 'ms;}';
			Qatrix.Qanimate.sheet.insertRule(rule, 0);
			...
			setTimeout(function ()
			{
				...
				if (properties['left'] || properties['top'])
				{
					var transform = $string.camelCase(prefix_name + 'transform'),
						offset = $offset(elem);
					$style.set(elem, transform, 'translate3d(' + ( properties['left'] ? (css_value['left'] - offset.left) + unit['left'] : '0' ) + ',' + ( properties['top'] ? (css_value['top'] - offset.top) + unit['top'] : '0' ) + ',0)');
					$style.set(elem, transform, '');
				}
				for (css in properties)
				{
					$style.set(elem, css_name[css], css_value[css] + unit[css]);
				}
			}, 50);
	...
	function (elem, properties, duration, callback)
	{
		...
		for (css in properties)
		{
			...
			if (properties[css].from != null)
			{
				...
				$style.set(elem, css_name[css], from_value + unit[css]);
				diff[css] = (css_value[css] - from_value) / p;
				ori[css] = from_value;
			}
			else
			{
				...
				{
					offset = $offset(elem);
					style_value = css === 'left' ? offset.left : offset.top;
				}
				else
				{
					style_value = parseInt($style.get(elem, css_name[css]));
				}
				css_value[css] = !$css.number[css] ? parseInt(properties[css]) : properties[css];
				unit[css] = $css.unit(css, properties[css]);
				$style.set(elem, css_name[css], $style.get(elem, css_name[css]));
				diff[css] = (css_value[css] - style_value) / p;
				ori[css] = style_value;
			}
		}
		for (; i < p; ++i)
		{
			$timer[i] = setTimeout(function ()
			{
				for (css in properties)
				{
					$style.set(elem, css_name[css], (ori[css] + (diff[css] * $step)) + unit[css]);
				}
				$step++;
			}, (duration / p) * i);
		}
