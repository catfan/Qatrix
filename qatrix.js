/*
    ...
    The Watrix project is still under MIT license.

    What changes have I made:
    1. Adjust the parameters of the $each,$id,$tag,$class' callback to (item, index, total)
    2. ?
*/
...
var
    ...
    mapcall = function (match, callback)
    {
        ...
        if (length !== undefined && length > 0)
        {
            for (; i < length; i++)
            {
                if (callback.call(match[i], match[i], i, length) === false)
                {
                    break;
                }
            }
        ...
        {
            return callback.call(match, match, 0);
        }
    },
    ...
    $each: function (haystack, callback)
    {
        ...
        if (is_object && (length - 1) in haystack)
        {
            for (; i < length; i++)
            {
                if (callback.call(haystack[i], haystack[i], i, length) === false)
                {
                    break;
                }
            }
        ...
        else if (is_object)
        {
            ...
            {
                callback.call(haystack[name], haystack[name], name, length);
            }
        }
        else
        {
