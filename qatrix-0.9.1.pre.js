...
var Qatrix = {
	...
	$storage = {
		...
		function (name, value)
		{
			$data.set(Qatrix.Qstorage, name, value);
			Qatrix.Qstorage.save('Qstorage');
		},
		get: localStorage ?
		function (name)
		{
		...
		function (name)
		{
			Qatrix.Qstorage.load('Qstorage');
			return $data.get(Qatrix.Qstorage, name);
		}
	},
	$event = {
	...
	function (elem, properties, duration, callback)
	{
		...
		for (; i < p; i++)
		{
			timer = setTimeout(update, (duration / p) * i);
		}
		setTimeout(function ()
		{
			for(i = 0; i < length ; i++)
...
(function ()
{
	...
	if( localStorage )
	{
		$ready(function ()
		{
			...
			Qatrix.Qstorage = $append( document.body, storage);
		});
	}
})();